...
class Capas:
    ...
    def __init__(self, arq_csv, valor, diaEntrada, mesEntrada):
        self.arq_csv = arq_csv
        self.valor = valor
        self.diaE = diaEntrada
        self.mesE = mesEntrada
        ...
        with open(self.arq_csv, encoding='UTF-8', errors='ignore') as csv_file:
            ...
            for lnh in _csv:
                ...
                diaS = (dataSorteio[0:2])
                mesS = dataSorteio[2:4]
                anoS = dataSorteio[4:]
                datasort = f"{diaS.zfill(1)}/{mesS.zfill(1)}/20{anoS}"

                #  AJUSTAR DATA DA ENTREGA PARA -8 DIAS A DATA DO SORTEIO
                ## Data Entraga
                # diaE = int(diaS)-8
                # mes = mesS
                # ano = anoS
                # dataEntraga = data = f"{str(diaE).zfill(1)}/{mes}/20{ano}"

                diaE = self.diaE
                mes = self.mesE
                ano = 22
                dataEntraga = data = f"{str(diaE)}/{mes.zfill(2)}/20{ano}"
                ...
                self.dados(datasort, etapa, codPonto, ponto, telefone, rota, endereco, cidade, seq,
    ...
    def dados(self, data, codPonto, etapa, ponto, telefone, rota, endereco,
              cidade, seq, inicio, final, qtd, distribuidor,
              tel_distribuidor, observPonto, dataEntrega):
        # Data Entrega
        self.pdf.set_text_color(255, 0, 0)
        self.pdf.text(32, 33.5, dataEntrega)
        # Data Sorteio

        self.pdf.text(115, 33.5, data)
...
if __name__ == '__main__':
    for f in listdir('/home/vitorluiz/Documentos/capas/csv'):
        print (f)
        Capas(f"/home/vitorluiz/Documentos/capas/csv/{f}", 25, '12', '06')